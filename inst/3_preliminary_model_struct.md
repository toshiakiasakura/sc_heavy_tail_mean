...
> (`gamma_sar` / `log_gamma_sar`, `build_ngm(…; gamma_sar=…)`, prior $\mathrm{Normal}(\log 0.33, 0.56)$).
...
interior to the softclamp $[\log 0.02,\log 5]$. Stage 2 returns the generated quantities
...
- **Configuration** (`FrameworkConfig`): $d_{\max}=240$, $w_{\text{group}}=2.5/240$,
  ...
  $\log\gamma_{\mathrm{SAR}}\sim\mathcal N(\log0.33,0.56^2)$ (`gamma_sar_prior`),
...
- **Transmission block** — *partially addressed*: the level is now an explicit, data-identified
  ...
  **Stage 2** of the cut (§6.0), conditioning on Stage-1 contact draws. *Remaining seams*: the relative-offset scales
