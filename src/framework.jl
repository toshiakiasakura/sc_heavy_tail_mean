...
const RHO_TIME_BOUNDS = (log(0.25), log(104.0))  # ρ_time — weeks (inert under the current prior)
...
Base.@kwdef struct FrameworkConfig
    ...
    gp_time_len_prior::Tuple{Float64,Float64}   = (8.5814, 15.6294)   # ⚠ (α, β) of an InverseGamma on ρ_time ITSELF — see `gp_len_prior` for the semantics change (`-ig`, 2026-08-06). Tail-matched to N(log 2, 0.35²): achieved 5%=1.1246, 50%=1.8944, 95%=3.5569 (targets 1.1246/3.5569, exact). β = β_spatial/10; α shared. ⚠ THIS IS THE RISK POINT OF `-ig`. ρ_time drifting to 20–27 weeks over a 12-week window is this model's recurring failure, and InverseGamma's polynomial right tail is heavier than the log-normal's: measured P(ρ_time>4 wk) 2.38e-02 → 2.76e-02 (1.2×), P(>11 wk, the window extent) 5.56e-07 → 4.04e-05 (73×), P(>26 wk) 1.17e-13 → 5.20e-08 (446 000×). In ABSOLUTE terms ~1 in 25 000 past the window, which is why tail-matching was judged tight enough — but it is 5 orders looser than before out at 26 wk. WATCH AT EVERY REFIT: the ρ_time posterior. ⚠ MEASURED AT THE REFIT (2026-08-06): ρ_time went to 66.2 / 47.3 wk for hurdle-Weibull (from 26.8 / 20.4 under the log-normal) and stayed at 2.24 / 2.16 for NegBin. That is NOT a prior-calibration failure and α must NOT be raised to "fix" it: ρ_time was already +7σ into the OLD prior's tail, so the drift predates `-ig`; the heavier tail merely stopped the prior fighting the likelihood. The decisive evidence is that ρ_time's own ESS TRIPLED (76–80 → 188–278) and sub-100 coordinates fell 38 → 10 overall — the prior–likelihood conflict was costing mixing everywhere. Raising α would buy a passing number by fighting the data. The real finding is that the hurdle-Weibull likelihood wants near-constant contacts over a 12-week window (tasks/todo.md open items 1/3), while NegBin does not. The previous docstring's claim that the clamp is inert and the prior is the sole restraint still holds (RHO_TIME_BOUNDS sits far into the tail), but verify it against a chain rather than asserting it — that claim was measured FALSE once already at (log 4, 0.5).
...
puts the posterior past ~4 wk, CHECK ρ_time's ESS before blaming the prior — at the 2026-08-06
refit it went to 66.2/47.3 wk (hweibull) while its ESS TRIPLED, which says the OLD prior was
fighting the likelihood, not that the new one is too loose. See `gp_time_len_prior`. Conversely the LOWER
