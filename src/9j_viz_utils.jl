...
(total-age, solid) and ρ_gap (age-gap, dashed) in **age-years**, and a TEMPORAL panel with the AR(1)
coefficient φ, which is **dimensionless** and lives on (0,1), each with a 90% ribbon. φ is absent
(NaN, not plotted) for pooled chains. Configs with NO Stage-1 contact fit at all (the NULL model)
have no length-scales and are skipped entirely rather than drawn as blank panels.

The temporal panel marks **φ = 1**, the pooled limit at which contacts are constant across the
window and the parameter stops being identified — so a chain sitting on the boundary reads as such
rather than merely as "near the top". That matters here: the hurdle-Weibull path has run to this
limit under every parameterisation tried (see `ar1_phi_prior`).
"""
function plot_lengthscales(rho, labels4, origins; h::Integer = 1)
    ...
    # age-years (0–50); column 3 is the AR(1) coefficient φ ∈ (0,1), dimensionless. Drawing all
    # three on one axis (the state until 2026-08-06) mixes units on a single "age-yrs / weeks"
    # label, and φ would render as an invisible flat line along the bottom of a 0–50 range —
    # misleading rather than merely wrong. The split survives whichever temporal parameterisation
    # is current: it was kept through the `-m32t` round trip of 2026-08-10, because weeks and
    # age-years never belonged on one axis either.
