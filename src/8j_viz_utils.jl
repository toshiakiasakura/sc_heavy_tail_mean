...
`rho_diag`/`rho_time[d] = exp(softclamp(log_rho_diag|log_rho_time,…))`
(diagonal/total-age and — in the separable spatio-temporal regime — temporal directions;
...
function load_transmission_draws(lbl::AbstractString, origin::Date, h::Integer;
    ...
    if chn === nothing                                                   # NULL model: no contact fit
        nan1 = fill(NaN, 1)
        return (; susc, inf, gamma_sar, F = pooled.F, rho_diag = nan1, rho_time = nan1,
                  w_mu = pooled.w_mu, w_sigma = pooled.w_sigma)          # GI/F are Stage-2, always there
    end
    s1names = string.(names(chn, :parameters))
    # Pre-`-diag` chains (2026-08-05) carry `log_rho_gap` from the anisotropic two-length-scale
    # kernel. `rho_diag` means something different there, so reporting it beside current-token
    # length-scales would silently mix generations. Same guard as `reconstruct_mu_draws`.
    if "log_rho_gap" in s1names
        @warn "Stage-1 chain carries `log_rho_gap` (pre-`-diag` anisotropic kernel); ρ set to NaN" s1p
        nan1 = fill(NaN, 1)
        return (; susc, inf, gamma_sar, F = pooled.F, rho_diag = nan1, rho_time = nan1,
    ...
    return (; susc, inf, gamma_sar, F = pooled.F, rho_diag, rho_time, w_mu, w_sigma)
end
