...
nuts_draws, phi_init_scale, phi_pf_max, phi_pf_override)` — temp file then rename, so a killed
process cannot leave a truncated artefact that the `isfile` skip counts as done). Idempotent
skip ⇒ resumable prefit.
