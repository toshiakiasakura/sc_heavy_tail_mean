...
**Both GP kernels are MATÉRN 3/2, and the spatial kernel smooths BOTH directions** (`-m32`, 2026-08-05). The age pair is rotated 45° into `u = (mid_a+mid_b)/√2` (total age) and `v = (mid_a−mid_b)/√2` (age gap); `Kp[m,n] = m32(|Δu|/ρ_diag)·m32(|Δv|/ρ_gap)` with `m32(x) = (1+√3x)·exp(−√3x)` — a separable product of two 1-D Matérn 3/2 factors, so it is unit-diagonal by construction and `ρ_diag = ρ_gap` does **not** give an isotropic Matérn (that identity held only for the old squared exponential). `Kt` is a 1-D Matérn 3/2 over the window weeks. Both `rho_diag` and `rho_gap` draw from the single `gp_len_prior`. **Why the kernel family changed**: the squared exponential's eigenvalues decay super-exponentially, so `Kt` went numerically rank-4-of-12 as ρ_time drifted and `z[:,1]` got pinned ~60× tighter than the other columns — the geometry behind 100 % max-tree-depth and min ESS 1.9–5.4/500 in the 2026-08-05 NUTS pilot. Measured: `Kt` keeps **full rank 12 at every ρ_time** in `RHO_TIME_BOUNDS`; `Ap` keeps rank 27 across all of `RHO_BOUNDS` (min eigenvalue 2.5e-2 at the `gp_len_prior` mode vs 2.9e-5 for SE, and 2.1e-3 vs **1.5e-8** — below the 1e-6 jitter — at +2σ); `cholesky(Ap + 1e-6·I)` clean at all 625 points of a 25×25 (ρ_diag, ρ_gap) grid. This **supersedes `-diag`**, which for a few hours the same day dropped `log_rho_gap` (389/977) and did not fix the mixing problem. ⚠ The ρ priors moved with it — then moved again under `-ig`, see below — and the tight time prior is what keeps ρ_time inside the identified region; the changes are complementary, don't revert one alone.

**All three GP length-scales have INVERSE-GAMMA priors on ρ itself** (`-ig`, 2026-08-06, user request). `rho_diag ~ InverseGamma(8.5814, 156.2941)`, `rho_gap` likewise (they share `gp_len_prior`), `rho_time ~ InverseGamma(8.5814, 15.6294)`. Sampled DIRECTLY — Turing's bijector supplies positivity and the Jacobian — so the chain stores the **constrained** ρ and the sampled columns were **renamed `log_rho_*` → `rho_*`**; every read-only mirror therefore does `exp(_softclamp(log(rho), …))`, taking `log` first. ⚠ The latent count is **unchanged** at 389/977, so the count can no longer date a chain; the in-chain signal is the name (`rho_gap` ⇒ `-ig`, `log_rho_gap` ⇒ `-m32`/`-t0`, absent ⇒ `-diag`). **Calibration is TAIL-MATCHED** to the log-normals it replaces — the 5 %/95 % points reproduce [11.246, 35.569] and [1.1246, 3.5569] exactly (α shared because both old priors had σ = 0.35 ⇒ the same 95/5 ratio; only β scales, β_t = β_s/10) — so the family changes while location and spread do not. ⚠ **The right tail is heavier** (polynomial ∝ρ^−α−1): P(ρ_time > 11 wk) 5.6e-07 → 4.0e-05, P(> 26 wk) 1.2e-13 → 5.2e-08. Absolutely still ~1 in 25 000 past the window, but ρ_time drifting to 20–27 wk is this model's recurring failure, so the refit was watched for it. ⚠ **MEASURED**: ρ_time went to **66.2 / 47.3 wk** for hurdle-Weibull (from 26.8 / 20.4) and stayed at 2.24 / 2.16 for NegBin — but **do NOT raise α**. ρ_time was already +7σ into the OLD prior's tail, so the drift predates `-ig`; the heavier tail merely stopped the prior fighting the likelihood, and the proof is that ρ_time's own ESS **tripled** (76–80 → 188–278) while sub-100 coordinates fell **38 → 10** and divergences 1 → 0. The real finding is that **the hurdle-Weibull likelihood wants near-constant contacts over a 12-week window** while NegBin does not. The lower tail is much thinner, which is the point: P(ρ_diag < 5) 3.7e-05 → 4.5e-07.
