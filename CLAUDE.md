...
**The structure field is SUM-TO-ZERO over the 28 age pairs within each week** (`-s0`, 2026-08-05). `R = η·(Q·La·z·Ltᵀ)` with `Q = _sum_zero_basis(28)` the constant Helmert basis of 1^⊥ and `La = chol(Qᵀ·Kp·Q + 1e-6·I)`, so `Cov(vec R) = η²·(Kt ⊗ M·Kp·M)` — the same GP *conditioned*, not approximated. Without it the field's per-week mean was a second copy of `c_t` and `η`/`σ_c` were confounded (worse as `ρ` grows; under the pre-`-diag` anisotropic kernel `Kp → J`, rank-1, under `-diag` it is `block(J₇, I₂₁)`, rank 22). `z` is `27×Tn`, so Stage 1 is **390** (NegBin) / **978** (hurdle-Weibull) latents — **389/977** since `-diag` dropped `log_rho_gap` later the same day. Two consequences: `η` is no longer exactly the marginal SD (the field's SD is `η·sqrt(diag(M·Kp·M))` — measured ×0.98–×0.93 over the prior's ±2σ ρ range under `-diag`, so `gp_scale_prior` was left alone), and **do not renormalise `Ap` by `tr(Ap)/P` to "restore" it** — as `ρ→∞` that is dominated by the jitter and degenerates the field to white noise, inverting the correct limit (field → 0, `c_t` carries everything).

**The spatial GP smooths the MATRIX DIAGONAL ONLY** (`-diag`, 2026-08-05). The 45° rotation into `u = (mid_a+mid_b)/√2` (total age) and `v = (mid_a−mid_b)/√2` (age gap) survives, but `v` no longer carries a length-scale — it only selects the diagonal, since `v = 0 ⟺ a = b`. `Kp[m,n]` is `1` on the diagonal of the matrix-of-pairs, `exp(-(u_m−u_n)²/(2ρ_diag²))` when **both** `m` and `n` are same-age cells `(a,a)`, and `0` otherwise. So `log_rho_gap` is gone and `log_rho_diag` is the sole spatial length-scale: the 7 diagonal cells are smoothed along total age, the 21 off-diagonal cells keep the marginal scale `η` but are correlated with nothing (ρ is identified by those 7 cells' 21 pairs). PSD and unit-diagonal at every ρ; `rank(Ap) = 27` up to ρ = 66.7 (**+5.6σ** under `gp_len_prior`), degrading to 24 at the ρ = 500 ceiling as the 7 diagonal cells merge, with `cholesky(Ap + 1e-6·I)` clean throughout. ⚠ **Do NOT "simplify" this by deleting the `v` term instead** (`ρ_gap → ∞`): that correlates cells by equal *total age*, drops `rank(Ap)` 27→21 and makes `2-10|16-24` identical to `11-15|11-15`. `gp_len_prior` was deliberately **left at `(log 4, 0.5)`** — see its docstring for what to watch at the first refit.
