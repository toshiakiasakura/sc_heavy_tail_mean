...
src/
  ...
  degree_agepair.jl          Age-pair (7×7 CIS bin) per-cell weekly contact-degree assembly (reuses 7j binning).
                               `assign_age_bin(a, b, grid, rng)` — the population-weighted age-interval → CIS-bin rule
                               — was lifted out of `prepare_degree_data`'s closure on 2026-09-04 so 14j could reuse it
                               (verified bit-identical at h=1 and h=4). ⚠ It CONSUMES `rng`, so the two call sites
                               inside `prepare_degree_data` (participants, then contacts) must keep that order.
  ...
  10j_viz_utils.jl           Model diagnostics — μ/dispersion reconstruction from Stage-1 chains (stays on the original four combos); `_read_disp_chain` carries a legacy `-hd` branch for the retained hierarchical chains.
                               Also the **Stage-1 GP hyperparameter mirror** (§7, 2026-08-10): `reconstruct_gp_hyper_draws` (per-draw ρ_diag/ρ_gap/φ/η/σ_c — `contacts` has NO DEFAULT, deliberately), `_prior_hyper` (the ONE place prior↔parameter is written down, `band` pushed through the model's own soft clamp), `collect_gp_hyper`, `gp_hyper_table` (+CSV), `plot_gp_hyper_horizons`, `plot_gp_hyper_marginals`.
  ...
  14j_viz_utils.jl           PUBLICATION figures (2026-09-04; §5/§6 added 2026-09-10). Blocks: `audit_stage1_grid` (the REPLACEMENT for
                               ...
                               SILENTLY wrong (y = −2 beside y = 0.01). 14j needs discrete counts, continuous
                               weighted degrees and both fitted curves together, so everything in it uses the
  ...
  <N>j_*.ipynb               Numbered notebook entry points:
                               ...
                               9j forecast diagnostics/scoring · 10j model diagnostics (§1–§6 + §7 Stage-1 GP
                                 hyperparameters vs prior) · 11j weekly identifiability + shrinkage ·
                               ...
                               13j model diagnostics, h=1 ONLY — a variant of 10j for generations fitted at a single
                                 ...
                                 ⚠ **13j now LAGS 10j by one section** (2026-08-10): 10j gained §7 and `tmp/build_13j_nb.py`
                               ...
                               14j PUBLICATION figures (2026-09-04; §5/§6 added 2026-09-10) — the only notebook here that is NOT a diagnostic.
                                 ...
                                 as chains are inserted, and renders "no chains on disk" rather than a blank axis if a
                                 path is empty. It ALSO skips origins failing `_p0_draws_valid`, but records them and
...
dt_intermediate/             Fitted Turing chains (JLD2): date-stamped dist fits + 8j_s1_*/8j_s2_* forecast artefacts; cached df_dds*.csv
                               ⚠ **RE-VERIFIED 2026-09-04 by `audit_stage1_grid` (14j §0), on the COMPLETE grid.**
                               It holds **2016 files: 504 `8j_s1_*` + 1512 `8j_s2_*`**, all under the CURRENT token
                               `temporal-w8h-lc0-nuts`. (It briefly held only 321 s1 and no s2 mid-transfer that same
                               day; both degree models are now complete, so 9j is unblocked.) It does NOT hold
                               `df_dds.csv` / `df_dds_settings.csv` — the loaders that cache them simply rebuild.
                                 • NegBin         252/252 — 63 origins × h1–4, 2020-10-18 … 2021-12-26
                                 • hurdle-Weibull 252/252 — same span
                               Structure matches the current `model_degree` exactly (`phi_time`, `log_rho_gap`, z 27
                               rows, z_c = Tn−1, npar 5+32Tn / 5+81Tn at Tn = n_fit+h). Provenance is uniform across
                               all 504 — `nuts / mooncake / 0.95 / 1000 adapts / 2000 draws / phi_init_scale 0.1` —
                               with **0 divergences**, min ESS 142.4 and max tree depth 9 against the cap of 10.
                               ⚠ `phi_pf_max`/`phi_pf_override` are **`:ABSENT` on all 504**: the grid predates the
                               2026-08-11 φ-boundary guard. Uniformly so — one generation, chains interchangeable.
                               Anything refitted later with current code WILL carry those keys and mix the grid on
                               them; re-run 14j §0 and decide deliberately rather than by accident.
                               ⚠ **A CHAIN CAN BE THE RIGHT SIZE AND SHAPE AND STILL BE PARTLY UNWRITTEN.** During the
                               2026-09-04 transfer, `8j_s1_weighted-hweibull_…_2021-02-14_h1.jld2` arrived with **61 of
                               its 441 `p0f` coordinates identically 0.0 across all 2000 draws** — an unwritten array
                               region — while `μ`, `κ`, the GP scalars and the other `p0f` cells were healthy. `isfile`
                               passed (normal size), the structural audit passed (dimensions right) and
                               `_stage1_gp_generation` passed (names right). Downstream, `K1 = (1−p0)·μ` then silently
                               contributed the full `μ` for every dead cell and inflated 14j §4's child total ×2.5,
                               reading as a real epidemiological signal three weeks before schools actually reopened.
                               This is the "truncated artefact counts as complete" hazard in CONTENT rather than
                               length. `_p0_draws_valid` (`14j_viz_utils.jl`) is the detector — a `p0f` coordinate
                               identically 0 or 1 over every draw cannot be a Beta posterior — and the re-transferred
                               grid is clean (0 of 63 h1 chains). Any new reader of `p0f` should call it.
                               Of the previous generations only ONE survives in the tree:
                                 dt_intermediate_ar1/          504 s1 + 1512 s2, CONTACTS_TOKEN_AR1 (= CONTACTS_SAVE_DIR_AR1)
                                                               — Pathfinder, `-t0-ar1`
                               `dt_intermediate_superseded_20260810/`, `_old/`, `_oldinit_nuts/`, `_hierarchical{,_ver1}/`,
                               `_age_block_ver1/`, `_ar1_uniformphi/`, `_GP_RBP/` and `nuts_pilot*/` are **all gone**, so
                               `CONTACTS_SAVE_DIR_HD` and the `-hd` readers (`plot_within_block_sd`,
                               `reconstruct_tau_draws`) now point at a directory that does not exist. Always check the
                               directory as well as the token.
                               ⚠ THE SAME TOKEN CAN NAME TWO INCOMPATIBLE GENERATIONS. The archived `temporal-w8h-lc0`
                               set (03:02) and the grid fitted from 14:37 carry an IDENTICAL token, identical parameter
                               names and identical dimensions — they differ only in `stage1_phi_init_scale`, which
                               landed at 06:13 and is deliberately not in the token. `prefit_stage1!` skips existing
                               artefacts, so leaving them in place would have silently reused 24 of 504 old-init chains.
                               They were moved aside, not overwritten, exactly as the field's docstring requires.
                               Stage-1 artefacts now record `phi_init_scale`, and `audit_stage1_grid` audits it: an
                               older file tallies `:ABSENT`, so a mixed grid fails the gate.
...
hpc/                         LSHTM HPC (Slurm + Singularity) layer for the 8j grid — spec:
                               B4a07_lang_Slurm_singularity_LSHTM_hpc.docx. Fitting ONLY; 9j–13j stay local.
                               Dockerfile.hpc      the HPC image: NOT the devcontainer (no Node/Claude Code/RTK),
                                                     adds R scoringutils, and BAKES the instantiated+precompiled
                                                     depot into /opt/julia world-readable (Singularity runs as the
                                                     HOST user, and compute nodes have no network).
                                                     ⚠ REBUILD WHENEVER Manifest.toml CHANGES — /workdir supplies
                                                     Project/Manifest from the bind mount, the packages come from
                                                     the image, and a disagreement wants a resolve that cannot run.
                               build_image.ps1     Windows: docker build + docker2singularity → .simg (docx §1-3-3)
                               config.sh           the ONLY file to edit: account, PROJECT_DIR, IMAGE, and the
                                                     generation selectors (STAGE1_USE_NUTS / FIT_END / ORIGIN_MIN)
                               s1_array.slurm      Stage 1: --array=0-15, 8 cpus, 64gb, 24h
                               s2_array.slurm      Stage 2: --array=0-15, 16 cpus, 32gb, 6h
                               run_grid.sh         in-container supervisor — relaunches 8j_run_grid.jl per its
                                                     0/10 exit-code protocol (the process boundary is the only
                                                     thing that resets the 1.08 GiB/origin climb)
                               README.md           the runbook, and the two DELIBERATE departures from the docx
```
...
**Fit path**: `fit_stage1` runs **NUTS** (`cfg.stage1_use_nuts`, the default since 2026-08-05), initialised from the Pathfinder mean via `_pf_mean_init` — so Pathfinder still runs first and the cost is **additive**; set `stage1_use_nuts=false` for the Pathfinder-only preliminary. ⚠ **One latent is exempt from that hand-off: `phi_time`** (`cfg.stage1_phi_pf_max = 0.9999`, 2026-08-11, user request). φ is logit-linked, so a Pathfinder mean at the boundary is not just uninformative but unusable as a start — the grid maximum 0.99999851 is `logit` **13.4**, and exactly 1.0 is `logit` `Inf`, which makes the logjoint non-finite and **fails the cell outright**. At or above the threshold φ is replaced by the same `logistic(N(0, stage1_phi_init_scale²))` ≈ 0.5 draw `_stage1_init` uses; *nothing else in the mean is touched*, because the firing cells are extreme in φ only (`log_eta` −0.79/−1.46/−1.62 vs a grid median of 0.21 — not the −238 collapse signature). Measured on the complete 504-chain Pathfinder grid: **3 cells fire at 0.9999** (12 at 0.999, 38 at 0.99, 0 at 0.999999), **all weighted-hurdle-Weibull at h=1** — NegBin fires 0/252. The distribution has NO cliff at 1.0, so the threshold is a judgement about tolerable logit starts, not a detector. NUTS path only (`_pf_mean_init` is never called otherwise), so the Pathfinder generation is unaffected. See `inst/3` §7 and `tasks/lessons.md`. NUTS is configured explicitly (`cfg.stage1_nuts_adapts=1000`, **`_draws=2000`** (500 until 2026-08-07), `_target_accept=0.95`, `_max_depth=10`) because a bare `NUTS()` would derive only 125 warmup iterations in 293–389/734–977 dimensions. One chain per fit ⇒ **no R̂**; health comes from `_nuts_diagnostics` (divergences, `frac_at_max_depth`, `min_ess`). The 500→2000 raise (user request, for the formal 63-origin run) scales only the sampling leg: measured per-fit total goes 1118/1436/1779/1930 s → ~1692/2509/2806/3027 s (**×1.60**, 219→351 CPU-h over the 504-fit grid), and buys ~4× ESS (pilot min 48.5–126.2 ⇒ ~195–505) plus the discriminating test for whether the split-R̂>1.01 failures are drift or autocorrelation. **Stage 2 is always Pathfinder and has no NUTS path at all** (100 cheap fits per Stage-1 draw — the point of the cut). Seed `cfg.seed = 1236`. Stage-1 gradients come from `cfg.ad_backend` (default `:mooncake`), Stage-2 from `cfg.stage2_ad_backend` (default `:reversediff`) — see the AD-backend paragraph above. Drivers: `prefit_stage1!` then `prefit_stage2!` (or `prefit_two_stage!`); forecast `two_stage_forecast`. Artefacts: `8j_s1_<degree>_<contacts>_<origin>_h<h>.jld2` (Stage-1 chain, key `result`, plus self-describing `sampler`/`diag`/`ad_backend`/`target_accept`/`nuts_adapts`/`nuts_draws`/`phi_init_scale`/`phi_pf_max`/`phi_pf_override` keys, no ngm token; audit them with **`audit_stage1_grid`** (`src/14j_viz_utils.jl`, 14j §0 — `tmp/check_grid.jl` left the tree with the rest of `tmp/`), which audits all of those for **uniformity** except `phi_pf_override`, a per-cell outcome that varies by design and is tallied only) and `8j_s2_<degree>_<ngm>_<contacts>_<origin>_h<h>.jld2` (Stage-2 pooled, key `pooled`). `CONTACTS_TOKEN` is now the SHORT `temporal-w8h-lc0-nuts` — the nine accumulated suffixes were dropped on 2026-08-09 (user request), since a token only has to separate the current generation from the retained ones. The complete 504/1512-file Pathfinder generation is reached by **`CONTACTS_TOKEN_AR1`** = `"temporal-gsar-cut-sc-p0-gi-s0-m32-t0-ar1"` **plus `CONTACTS_SAVE_DIR_AR1`** — it was moved to `dt_intermediate_ar1/`, so token-without-directory finds nothing (the `CONTACTS_TOKEN_HD` situation, not the `CONTACTS_TOKEN_PF` one). `dt_intermediate/` itself now holds 321 `8j_s1_*` under the CURRENT token and NO `8j_s2_*` (see the Repository Layout entry). All retained tokens are literals, because no `cfg` reproduces them. ⚠ Any guard that sniffed a suffix out of the token (`occursin("-m32", contacts)` in `reconstruct_mu_draws` / `load_transmission_draws`) must now go through **`is_legacy_token(contacts)`** — current-style tokens carry no historical markers, so a bare positive test rejects every current chain.
...
**Scoring** (`scoring.jl`): primary metric is **WIS** via R `scoringutils` v2 on quantile-format forecasts (`to_quantile_long` → `score_wis`); native sample CRPS is a cheap cross-check. WIS is reported on both the natural and the **log scale** — the latter is WIS after a log *transform* of forecasts and observations, and is the headline (`WIS_SCALE = "log"`). A separate **log score** (the logarithmic scoring rule, via `scoringutils`' sample class) existed 2026-07-30 → 2026-08-13 and was **removed** (user request); don't reinstate it by mistaking the surviving "log-scale WIS" for it. Outputs go to `res/8j_*`.
...
**The TEMPORAL correlation is AR(1)** (`-ar1`, 2026-08-06, user request; reverted to Matérn 3/2 by `-m32t` on 2026-08-10 and **restored the same day**). `Kt[s,t] = φ^|s−t|`, which *is* an AR(1) correlation matrix (equivalently the exponential / Matérn 1/2 kernel), in the **time direction only** — the spatial kernel is untouched. No structural change was needed: the separable matrix-normal already gives every age pair its own temporal trajectory under one shared amplitude `η` ("AR(1) per age pair, sharing the variance"), and the pairs stay correlated across age through `La`. The temporal parameter is **`phi_time`**, sampled `Beta(cfg.ar1_phi_prior...)` = **Beta(3,3)** since 2026-08-09 (Beta(2,2) 2026-08-08→09; **Uniform(0,1)** 2026-08-06→08, which was MEASURABLY BROKEN — 147/252 (58%) hurdle-Weibull Stage-1 Pathfinder fits DIVERGED with φ at the boundary, median 0.9992, and `log_eta` at −238; NegBin untouched at 0/252, φ ≤ 0.844. See `tasks/lessons.md`; `inst/3` §11 keeps the standing caveat). ⚠ **Beta(3,3) has never been fitted at scale** — it was set the day the `-w8h-lc0` grid was killed at one file.
