...
src/
  ...
  10j_viz_utils.jl           Model diagnostics — μ/dispersion reconstruction from Stage-1 chains (stays on the original four combos); horseshoe mirrors (`reconstruct_rhs_components`, `plot_rhs_globals`) + a legacy `-hd` branch
  tune_tau0.jl               τ₀ tuning driver — fits ONE origin per τ₀ step and reports the MEASURED escape (τ, c, λ, multiplier, m_eff, within-block SD, #cells with δ CI excluding 0). Not part of any preamble; `include` it alongside 8j/10j/11j_viz_utils.jl
  11j_viz_utils.jl           Weekly identifiability (mean vs neighbourhood moment timelines) + regularised-horseshoe SHRINKAGE panels (escape fraction, m_eff vs prior-predictive, ranked RE, escape-vs-n_pos, old-vs-new within-block SD)
...
Contacts are **per-week** by default (`constant_contacts=false`, the notebook setting): the age-pair mean is estimated separately each window week by a separable spatio-temporal GP (per-week level `c_t` + matrix-normal field, shared `ρ_diag/ρ_gap/ρ_time`, `η`, `σ_c`), so the renewal `N(t)` varies through that week's `C*_t` as well as antibody.

**Dispersion is a REGULARISED HORSESHOE** (`inst/3` §4.3; Piironen & Vehtari 2017, arXiv:1707.01694 eq. 11; 2026-08-02). Per ordered cell × week, `log d_{ij,t} = β[bl,t] + τ·λ̃_{ij,t}·z_{ij,t}` with `λ̃² = c²λ²/(c²+τ²λ²)` — a **window-global** `tau ~ N⁺(0, τ₀²)` with **τ₀ set per degree family** via `disp_tau0_prior(cfg, dm)` (`disp_re_scale_prior_unweighted` / `_weighted`, both 0.01 as of 2026-08-02; it was one shared `τ_t` per week before), a per-cell-per-week local scale `lam ~ half-t₃(0,1)` (`A²×Tn`), and a scalar slab `c2 ~ InverseGamma(ν/2, νs²/2)` with `ν=4`, `s=1`. `d` is the NegBin dispersion `φ` or the Weibull shape `κ`; `β` stays the 4×Tn child/adult block mean. The multiplier `τλ̃ = c·u/√(c²+u²)` (`u = τλ`) is bounded by `c`, so `|δ| ≤ c|z|` and the composed value no longer reaches the soft-clamp. Stage-1 latent count: 1580 (NegBin) / 2168 (hurdle-Weibull). `constant_contacts=true` recovers the pooled one-`C*`-per-window preliminary. `model_degree` branches on the flag but returns per-week moments `(; K1, K2, G)` (length `Tn`) either way; the forecast NGM uses the origin-week slice via `Cstar_end[m]` in the Stage-2 pooled result.
...
- **Turing + Weibull/exp over/underflow**: `κ = exp(log_kappa)` can go non-finite on aggressive Pathfinder/optim steps → `DomainError: Weibull α>0`. Log-parameters (`log_kappa`, `log_k`, `log μ` in `model_degree`; `log_gamma_sar` in `model_transmission`) are soft-**clamped** (`_softclamp`); the mode stays interior so gradients are unaffected. Keep those clamps when editing either model. **`_softclamp` must stay the Inf-safe nested form** `lo + softplus((hi − softplus(hi − x)) − lo)` — the naive `x − softplus(x−hi) + softplus(lo−x)` returns **NaN** at `x=±Inf` (`Inf − Inf`), so when a raw log-latent (`log_kappa`/`log_k`) or the GP field overflows to Inf, `κ=exp(softclamp(Inf))` becomes NaN and the Weibull throws (aborting the whole fit). This bit the **neighbourhood** NGM hardest (its `gamma(1+2/κ)` second moment pushes the optimiser to extremes); see `tasks/lessons.md` 2026-07-11. **`_softcap(x, hi) = hi - softplus(hi - x)`** is the upper-only sibling, for the horseshoe local scale `lam` (already `>0`): do **not** use `_softclamp(λ, 0, hi)` there — near `lo=0` it distorts (`_softclamp(0.7,0,1e6) = softplus(0.7) = 1.10`), putting a floor of ≈log 2 under λ and destroying the shrinkage.
- **τ₀ cannot be set analytically, and it is PER FAMILY.** Piironen & Vehtari's `τ₀ = p₀/(D−p₀)·σ/√n` assumes a linear model with a residual scale `σ` and sample size `n` — a NegBin / hurdle-Weibull likelihood on counts and durations has neither, so the formula does not transfer. A prior-predictive Monte Carlo of `m_eff` (`prior_shrinkage_reference`) describes only the *prior*, and measured at τ₀=0.1 the posterior τ sat 7–15 prior SDs out, i.e. the likelihood overwhelms it. **Measure the realised escape from fitted chains** — `src/tune_tau0.jl` fits one origin per τ₀ step and reports τ, c, λ, the multiplier, escape, `m_eff`, within-block SD and the count of cells whose δ CI excludes 0. The two families need different values (measured multiplier 1.61 negbin vs 0.73 hweibull), so read τ₀ only through `disp_tau0_prior(cfg, dm)`.
- **`contacts_label` now encodes both τ₀ values** (`_tau0_tag`), so tuning steps write distinct filenames and sit side by side on disk. Without that, `fit_or_load_stage1`'s `isfile` short-circuit silently reloads the previous step's chain — the footgun `tasks/lessons.md` records twice. Consequence: `CONTACTS_TOKEN` is a load-time const from the **default** config and goes stale during tuning, so every helper that takes a `cfg` defaults to `contacts_label(cfg)` instead; use `CONTACTS_TOKEN` only where no `cfg` is in scope.
- **The horseshoe multiplier must be written `c*u/sqrt(c^2+u^2)`, and this is an AD constraint, not style.** Measured with `ReverseDiff.gradient`: the literal `c²λ²/(c²+τ²λ²)` NaNs when `λ²` overflows; an `exp(_softclamp(log(u), …))` guard NaNs *in the gradient* at `u=0` (`d log u/du → ∞`) and floors the value at 4.7e-14 instead of 0; `c*sqrt(w)` with `w = u²/(c²+u²)` NaNs in the gradient at `w=0`. Only the adopted form is finite in value **and** gradient at both ends. The `u=0` corner is where a working horseshoe puts *most* cells, and all three bad variants return a perfectly finite **value** there — a "does the model evaluate?" smoke test does not catch them. Note `logpdf(TDist, x)`/`logpdf(Normal, x)` both go `-Inf` once `x²` overflows (`x > 1.34e154`); that is upstream of the composition, shared by every latent in the model, and not fixable by `_softcap`.
