...
**The TEMPORAL correlation is AR(1)** (`-ar1`, 2026-08-06, user request). `Kt[s,t] = φ^|s−t|`, which *is* an AR(1) correlation matrix (equivalently the exponential / Matérn 1/2 kernel), replaces the Matérn 3/2 in the **time direction only** — the spatial kernel is untouched. No structural change was needed: the separable matrix-normal already gives every age pair its own temporal trajectory under one shared amplitude `η` ("AR(1) per age pair, sharing the variance"), and the pairs stay correlated across age through `La`. `log_rho_time` → **`phi_time`**, sampled `Beta(cfg.ar1_phi_prior...)` = **Beta(2,2)** since 2026-08-08 (user request). It was **Uniform(0,1)** from 2026-08-06 to 2026-08-08 and that was MEASURABLY BROKEN: 147/252 (58%) hurdle-Weibull Stage-1 Pathfinder fits DIVERGED, with φ at the boundary (median 0.9992) and `log_eta` at −238 under a N(0,0.5²) prior; NegBin was untouched (0/252, φ ≤ 0.844). See `inst/3` §12 and `tasks/lessons.md`. ⚠ The latent count is **unchanged at 389/977**, so the count cannot date a chain; the in-chain signal is the name (`phi_time` ⇒ `-ar1`, `log_rho_time` ⇒ earlier) plus the token. **Why**: measured at *matched effective rank* (same temporal pooling), AR(1) gives `Kt` min eigenvalue 2.7e-05 → 5.1e-03 and `Lt` column spread 94.2 → 23.1 at effrank 1.08, and the LEVEL's `Lc` spread is essentially **flat at 1.6–1.8 across every φ** where Matérn 3/2 degrades to 13.5. Mechanism: AR(1) is **Markov** — tridiagonal precision, polynomially-decaying eigenvalues — so it keeps spectral mass in the non-constant directions even at φ = 0.995, where Matérn 3/2's spectrum has collapsed and `Lt`'s first column absorbs the field. It is a wash in NegBin's regime (ρ_time ≈ 2.2) and helps hurdle-Weibull, whose likelihood wants near-constant contacts. ⚠ This is a **modelling** change too: AR(1) paths are non-differentiable and memory is LONGER at long lag (at matched lag-1 0.785, lag-4 is 0.380 vs 0.140). ⚠ "A high φ is the measurement, not a failure" holds ONLY where the likelihood has something to say. With p⁰ ≈ 0.95 the hurdle-Weibull likelihood is nearly flat in the temporal direction, so φ→1 there is an unidentified parameter running to a degenerate boundary — do NOT use that maxim to wave through a boundary pile-up. The `Lc` "flat at 1.6–1.8 across every φ" figure above was measured at MATCHED EFFECTIVE RANK on `Kt`; the `-t0` level is built on the PROJECTED kernel `Qtᵀ·Kt·Qt`, whose φ→1 limit is exactly 0 regardless (`Qtᵀ·J·Qt = 0`), which is the actual failure mode. ⚠ `RHO_TIME_BOUNDS` and the temporal soft-clamp are gone from this path (φ ∈ (0,1) by construction, φ^k cannot overflow); the constant is retained only for replaying archived chains.
